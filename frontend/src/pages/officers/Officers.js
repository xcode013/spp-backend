import React, { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { getMe } from '../../middleware/addSlice';
import Layout from '../layout/Layout';
import Header from '../../components/header/Header';
import Table, { RowData, Data } from '../../components/table/Table';
import { getOfficers } from '../../api/api';
import Button from '../../components/buttons/Button';
import "./officers.css"
import { NavLink } from 'react-router-dom';
import SearchBar from '../../components/searchbar/SearchBar';
import axios from 'axios';

function Officers() {
    const [datas, setDatas] = useState([]);
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const {isError, user} = useSelector((state) => state.auth);

    useEffect(()=>{
        getOfficers().then((result) => {
            setDatas(result);
        })
    }, []);

    const deleteHandler = async(key) => {
        await axios.delete(`http://localhost:5000/officers/${key}`);
        getOfficers().then((result) => {
            setDatas(result);
        })
    }

    useEffect(()=> {
        dispatch(getMe());
    }, [dispatch]);
    
    useEffect(()=>{
        if(isError) navigate("/");
        if(user && user.level !== "admin") navigate("/dashboard");
    }, [isError, navigate, user])
  return (
    <Layout>
        <div className='officer-page'>
            <Header
                mainTitle={"Officers"}
                subTitle={"Page of Officers data"}
            />
            <main className='body'>
                <header className='body-head'>
                    <NavLink to={"/officers/add"} className={"add-button"}>
                        <Button
                            content={"Add Staf"}
                            color={"success"}
                        />
                    </NavLink>

                    <SearchBar/>
                </header>
                <Table
                    tableHead={["no", "username", "name", "level", "action"]}
                >
                    {
                        datas.map((data, index) => {
                            return(
                                <RowData key={data.id}>
                                    <Data textAlign={"center"}>{index + 1}</Data>
                                    <Data>{data.username}</Data>
                                    <Data>{data.name}</Data>
                                    <Data>{data.level}</Data>
                                    <Data textAlign={"center"}>
                                        <NavLink to={`/officers/update/${data.id}`}>
                                            <Button
                                                content={"Update"}
                                                color={"warning"}
                                            />
                                        </NavLink>
                                        <Button
                                            content={"Delete"}
                                            color={"danger"}
                                            actionOnClick={() => deleteHandler(data.id)}
                                        />
                                    </Data>
                                </RowData>
                            )
                        })
                    }
                </Table>
            </main>
        </div>
    </Layout>
  )
}

export default Officers